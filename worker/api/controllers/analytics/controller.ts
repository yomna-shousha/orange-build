...
export class AnalyticsController extends BaseController {
  ...
  async getUserAnalytics(
    ...
    try {
      ...
      if (!authUser) {
        return this.createErrorResponse<UserAnalyticsResponseData>('Authentication required', 401);
      }
      ...
      if (!userId) {
        return this.createErrorResponse<UserAnalyticsResponseData>('User ID is required', 400);
      }
      ...
      if (daysParam) {
        ...
        if (isNaN(days) || days < 1 || days > 365) {
          return this.createErrorResponse<UserAnalyticsResponseData>('Days must be between 1 and 365', 400);
        }
      }

      // Get analytics data
      const analyticsService = new AiGatewayAnalyticsService(env);
      const analyticsData = await analyticsService.getUserAnalytics(userId, days);
  ...
  async getAgentAnalytics(
    ...
    try {
      ...
      const authUser = this.extractAuthUser(context);
      if (!authUser) {
        return this.createErrorResponse<AgentAnalyticsResponseData>('Authentication required', 401);
      }
      ...
      if (!agentId) {
        return this.createErrorResponse<AgentAnalyticsResponseData>('Agent ID is required', 400);
      }
      ...
      if (daysParam) {
        days = parseInt(daysParam);
        if (isNaN(days) || days < 1 || days > 365) {
          return this.createErrorResponse<AgentAnalyticsResponseData>('Days must be between 1 and 365', 400);
        }
      }

      // Get analytics data
      const analyticsService = new AiGatewayAnalyticsService(env);
      const analyticsData = await analyticsService.getChatAnalytics(agentId, days);
